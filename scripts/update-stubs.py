...
def get_hatch_python_executable() -> str:
    ...
    try:
        ...
        print(f"Error: Could not find the Python executable in the Hatch environment. {e}", file=sys.stderr)
        sys.exit(1)
    ...
    for dep_string in unique_deps:
        ...
        if response.status_code == 200:
            ...
            if dist_name_key and dist_name_key in installed_packages:
                ...
                print(
                    f"  -> WARNING: Could not resolve '{pypi_name}' to an installed package. Skipping.", file=sys.stderr
                )
    ...
    if packages_to_stub:
        ...
        for pypi_name, (import_names, pinned_spec) in packages_to_stub.items():
            ...
            with tempfile.TemporaryDirectory() as temp_dir_str:
                ...
                try:
                    subprocess.run(
                        [hatch_python_executable, "-m", "venv", str(temp_dir / ".venv")],
                        check=True,
                        capture_output=True,
                        text=True,
                    )
                    ...
                    subprocess.run(
                        [pip_executable, "install", pinned_spec, "pyright"], check=True, capture_output=True, text=True
                    )
                    ...
                    print(
                        f"  -> ERROR: Failed to create stub for {pypi_name}. Reason: {error_message}", file=sys.stderr
                    )
...
def main():
    ...
    generate_parser = subparsers.add_parser(
        "generate", help="Check for community stubs and generate local ones if not found."
    )
