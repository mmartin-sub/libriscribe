...
from src.libriscribe2.validation import (
    ContentLocation,
    Finding,
    FindingType,
    Severity,
...
class AdvancedContentValidator(ValidatorBase):
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        logger.info(
            f"Initialized {self.name} with {len(self.validation_rules)} rules and {len(self.quality_thresholds)} thresholds"
        )
            ...
            findings.append(
                self.create_finding(
                    ...
                    metadata={
                        ...
                        "review_threshold": self.get_quality_threshold("human_review"),
                    },
                )
            )
    ...
    async def post_validation_hook(
        ...
        logger.info(
            f"Validation completed: {len(result.findings)} findings, quality score: {result.metrics.get('overall_quality_score', 'N/A')}"
        )
            ...
            return ValidatorResult(
                ...
                findings=[
                    self.create_finding(
                        ...
                        message=f"Data processing error occurred but was handled: {error!s}",
    ...
    async def on_configuration_change(self, old_config: dict[str, Any], new_config: dict[str, Any]) -> None:
        ...
        changed_rules = set(new_config.get("validation_rules", {}).keys()) - set(
            old_config.get("validation_rules", {}).keys()
        )
        if changed_rules:
...
async def demonstrate_validator_lifecycle():
    ...
    validator.configure_validation_rules({"require_citations": True, "check_plagiarism": False})

    # Update quality thresholds
    validator.configure_quality_thresholds({"citation_coverage": 90.0, "originality_score": 95.0})
    ...
    for i, test_case in enumerate(test_contents, 1):
        ...
        try:
            ...
            result = await validator.validate_with_lifecycle(test_case["content"], test_case["context"])
    ...
    try:
        result = await validator.validate_with_lifecycle(
            "error_test content", {"content_type": "test", "content_id": "error_test"}
        )
...
async def demonstrate_configuration_scenarios():
    ...
    print("   ✅ Professional validator configured")
    ...
    print("   ✅ Draft validator configured")
    ...
    print(
        f"   📚 Professional: {len(prof_result.findings)} findings, quality: {prof_result.metrics.get('overall_quality_score', 0):.1f}"
    )

    # Draft validation
    draft_result = await draft_validator.validate_with_lifecycle(test_content, test_context)
    print(
        f"   ✏️ Draft: {len(draft_result.findings)} findings, quality: {draft_result.metrics.get('overall_quality_score', 0):.1f}"
    )
...
if __name__ == "__main__":
    ...
    print("\n🎯 Key Takeaways:")
    print("   1. ValidatorBase provides comprehensive lifecycle management")
    print("   2. Lifecycle hooks enable custom preprocessing and postprocessing")
    print("   3. Error recovery mechanisms provide robust validation")
    print("   4. Dynamic configuration allows flexible validation rules")
    print("   5. Quality thresholds enable automated human review flagging")
    print("   6. Helper methods simplify finding creation and management")
