...
from src.libriscribe.validation import ValidationConfig, ValidationEngineImpl
from src.libriscribe.validation.ai_mock import AIMockManager, MockScenario
from src.libriscribe.validation.testing import (
...
async def demonstrate_ai_mock_best_practices():
    ...
    print(f"✅ Mock Response Structure:")
    ...
    print(f"✅ Interface is identical for real AI calls")
    ...
    print(f"✅ Playback successful - deterministic response")
    ...
    test_cases = await test_framework.create_comprehensive_test_suite(
        validators=validators_to_test,
        content_types=content_types_to_test
    )
    ...
    coverage_reporter = CoverageReporter(coverage_data_dir=".demo/coverage_data")
    ...
    bad_content = await test_data_generator.generate_known_bad_content(
        "chapter", "content_validator"
    )
    ...
    good_response = await mock_manager.get_ai_response(
    ...
    bad_response = await mock_manager.get_ai_response(
    ...
    print(f"   - Good content mock response: Appropriate high-quality response")
    print(f"   - Bad content mock response: Appropriate low-quality response")
    ...
    print(f"✅ Saved current test results as baseline for future regression testing")
    
    # Demonstrate regression testing (would compare with previous baseline)
    print(f"✅ Future test runs will compare against this baseline to detect regressions")
    ...
    print(f"📊 View detailed reports in: .demo/test_data/ and .demo/coverage_data/")
...
async def demonstrate_integration_with_validation_engine():
    ...
    print(f"✅ Validation engine initialized with mock AI enabled")
    
    # Create mock validator that uses AI mock system
    mock_manager = AIMockManager()
    
    from src.libriscribe.validation.interfaces import ValidatorBase, ValidatorResult, ValidationStatus
    ...
    class DemoContentValidator(ValidatorBase):
        ...
        async def validate(self, content, context):
            ...
            try:
                ...
                from src.libriscribe.validation.interfaces import Finding, FindingType, Severity
                findings = []
    ...
    print(f"✅ Mock validator registered with engine")
    ...
    print(f"✅ Validation completed using mock AI:")
...
if __name__ == "__main__":
    ...
    print(f"\n🎯 Key Takeaways:")
    print(f"   1. Mock AI responses maintain identical interfaces to real AI")
    print(f"   2. Comprehensive scenario testing covers all edge cases")
    print(f"   3. Record/playback enables deterministic testing")
    print(f"   4. Configuration flags allow seamless switching")
    print(f"   5. Coverage tracking ensures thorough testing")
    print(f"   6. Integration with validation engine is seamless")
