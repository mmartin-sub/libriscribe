...
If you want to install `libriscribe2` as a standalone command-line tool, you can use `uv tool install`. This is a convenient way to make the tool available system-wide without interfering with other Python projects.
...
libriscribe2 create-book --title="My Global Book" --mock --generate-concept
```
...
```bash
# Run a quick test to see if the CLI is working (generates concept only)
...
### Using Alternative Endpoints (LiteLLM, Llama.cpp, etc.)

You can use LibriScribe2 with any OpenAI-compatible API endpoint by setting the `openai_base_url` in your `config.json` file. This allows you to use services like LiteLLM or a locally running model server.

**Example for LiteLLM:**

If you are running the [LiteLLM proxy server](https://docs.litellm.ai/docs/simple_proxy), you can configure it as follows:

```json
{
  "openai_api_key": "your-dummy-key",
...
If you are running a local server with [Llama.cpp](https://github.com/ggerganov/llama.cpp), you can configure it like this:
...
When you create a new book, LibriScribe2 generates a dedicated project folder inside the `projects/` directory. This folder contains all the artifacts and logs for your book. Here's a breakdown of the key files you'll find:

```
projects/
└── your-project-name/
    ├── project_data.json    # Core project metadata and knowledge base.
    ├── outline.md           # The complete book outline generated by the OutlinerAgent.
    ├── characters.md        # Detailed profiles of all generated characters.
    ├── worldbuilding.md     # (Optional) Details about the book's world, if enabled.
    ├── chapter_1.md         # The generated content for each individual chapter.
    ├── chapter_2.md
    │   ...
    ├── manuscript.md        # The final, formatted manuscript.
    └── book_creation.log    # A detailed log of the entire book creation process for debugging.
```
