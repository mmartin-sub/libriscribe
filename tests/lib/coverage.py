...
This module provides coverage testing utilities.
"""
...
class CoverageReporter:
    ...
    def __init__(self, coverage_data_dir: str | None = None):
        self.coverage_data_dir = Path(coverage_data_dir) if coverage_data_dir else Path(".libriscribe2/coverage_data")
    ...
    async def generate_coverage_report(self, test_results: list[Any]) -> CoverageReport:
        ...
        recommendations = self._generate_recommendations(validator_coverage, scenario_coverage, content_type_coverage)
    ...
    def _analyze_validator_coverage(self, test_results: list[Any]) -> dict[str, CoverageMetrics]:
        ...
        validator_test_counts: dict[str, int] = {}
        validator_scenarios: dict[str, set[str]] = {}
    ...
    def _analyze_scenario_coverage(self, test_results: list[Any]) -> dict[str, CoverageMetrics]:
        ...
        scenario_validators: dict[str, set[str]] = {}
        scenario_test_counts: dict[str, int] = {}
    ...
    def _analyze_content_type_coverage(self, test_results: list[Any]) -> dict[str, CoverageMetrics]:
        ...
        content_type_validators: dict[str, set[str]] = {}
        content_type_scenarios: dict[str, set[str]] = {}
        content_type_test_counts: dict[str, int] = {}
    ...
    def _calculate_overall_coverage(
        ...
        content_type_avg = sum(m.coverage_percentage for m in content_type_coverage.values()) / len(
        ...
        total_items = len(self.expected_validators) * len(self.expected_scenarios) * len(self.expected_content_types)
    ...
    def _generate_recommendations(
        ...
        untested_validators = [
            validator_id for validator_id, metrics in validator_coverage.items() if metrics.coverage_percentage == 0
        ]
        if untested_validators:
            recommendations.append(f"Implement tests for untested validators: {', '.join(untested_validators)}")

        low_coverage_validators = [
            validator_id for validator_id, metrics in validator_coverage.items() if 0 < metrics.coverage_percentage < 50
        ]
        if low_coverage_validators:
            recommendations.append(f"Increase scenario coverage for validators: {', '.join(low_coverage_validators)}")

        # Scenario recommendations
        poorly_covered_scenarios = [
            scenario for scenario, metrics in scenario_coverage.items() if metrics.coverage_percentage < 50
        ]
        if poorly_covered_scenarios:
            recommendations.append(f"Add more validator tests for scenarios: {', '.join(poorly_covered_scenarios)}")

        # Content type recommendations
        untested_content_types = [
            content_type for content_type, metrics in content_type_coverage.items() if metrics.coverage_percentage == 0
        ]
        if untested_content_types:
            recommendations.append(f"Create tests for untested content types: {', '.join(untested_content_types)}")
        ...
        if overall_coverage < 60:
            recommendations.insert(
                0,
                "PRIORITY: Overall coverage is below 60%. Focus on basic test coverage first.",
            )
    ...
    async def generate_coverage_trend_report(self, days: int = 30) -> dict[str, Any]:
        ...
        for file_date, file_path in report_files:
            try:
                ...
                trend_data["overall_coverage"].append(report_data["overall_coverage"]["coverage_percentage"])

                # Calculate average coverage for each category
                validator_avg = sum(
                    metrics["coverage_percentage"] for metrics in report_data["validator_coverage"].values()
                ) / len(report_data["validator_coverage"])
                trend_data["validator_coverage"].append(validator_avg)

                scenario_avg = sum(
                    metrics["coverage_percentage"] for metrics in report_data["scenario_coverage"].values()
                ) / len(report_data["scenario_coverage"])
                trend_data["scenario_coverage"].append(scenario_avg)

                content_type_avg = sum(
                    metrics["coverage_percentage"] for metrics in report_data["content_type_coverage"].values()
                ) / len(report_data["content_type_coverage"])
                trend_data["content_type_coverage"].append(content_type_avg)

                trend_data["recommendations_count"].append(len(report_data["recommendations"]))
        ...
        if len(trend_data["overall_coverage"]) >= 2:
            ...
            trend_analysis = {
                ...
                "coverage_change_percentage": (coverage_trend / previous_coverage) * 100
