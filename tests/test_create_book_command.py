...
from create_book_command import (
    validate_title,
    validate_output_dir,
    validate_category,
    validate_genre,
...
class TestValidationFunctions:
    ...
    def test_check_required_parameters(self):
        ...
        # This would normally prompt the user, but we can't test that here
        # Just verify it doesn't raise an exception
        try:
            check_required_parameters(params)
        except Exception:
            pytest.fail("check_required_parameters raised an exception unexpectedly!")

class TestCreateBookCommand:
    @patch('create_book_command.ProjectManagerAgent')
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_success(self, mock_console, mock_settings, mock_project_manager):
        # Setup mocks
        mock_project_manager_instance = MagicMock()
    ...
    @patch('create_book_command.console')
    @patch('create_book_command.typer.prompt', return_value="Test Book")
    def test_create_book_missing_title(self, mock_prompt, mock_console):
        # Call the function with missing title
        result = create_book(title=None)
    ...
    @patch('create_book_command.console')
    @patch('create_book_command.typer.prompt', side_effect=typer.Abort())
    def test_create_book_abort_title_prompt(self, mock_prompt, mock_console):
    ...
    @patch('create_book_command.console')
    def test_create_book_invalid_chapters(self, mock_console):
        # Call the function with invalid chapters
        result = create_book(title="Test Book", chapters=-5)
        
        # Verify result
        assert result == EXIT_INVALID_ARGS
        mock_console.print.assert_any_call("[red]Error in command arguments: Number of chapters must be greater than 0[/red]")
    
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_env_error(self, mock_console, mock_settings):
        # Setup mock to raise exception
        mock_settings.side_effect = Exception("Invalid environment configuration")
        
        # Call the function
        result = create_book(title="Test Book")
        
        # Verify result
        assert result == EXIT_ENV_CONFIG_ERROR
        mock_console.print.assert_any_call("[red]Error loading environment configuration: Invalid environment configuration[/red]")
    
    @patch('create_book_command.ProjectManagerAgent')
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_llm_init_error(self, mock_console, mock_settings, mock_project_manager):
        # Setup mocks
        mock_project_manager_instance = MagicMock()
        mock_project_manager.return_value = mock_project_manager_instance
    ...
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_file_system_error(self, mock_console, mock_settings, mock_project_manager):
        # Setup mocks
        mock_project_manager_instance = MagicMock()
        mock_project_manager.return_value = mock_project_manager_instance
        mock_project_manager_instance.initialize_project_with_data.side_effect = Exception("File system error")
        
        # Call the function
        result = create_book(title="Test Book")
        
        # Verify result
        assert result == EXIT_FILE_SYSTEM_ERROR
        mock_console.print.assert_any_call("[red]Error initializing project: File system error[/red]")
    
    @patch('create_book_command.ProjectManagerAgent')
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_generation_error(self, mock_console, mock_settings, mock_project_manager):
        # Setup mocks
        mock_project_manager_instance = MagicMock()
        mock_project_manager.return_value = mock_project_manager_instance
        ...
        # Call the function
        result = create_book(title="Test Book", generate_concept=True)
        
        # Verify result
        assert result == EXIT_BOOK_CREATION_ERROR
        mock_console.print.assert_any_call("[red]ERROR during book creation: Generation error[/red]")
        
    @patch('create_book_command.ProjectManagerAgent')
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_network_error(self, mock_console, mock_settings, mock_project_manager):
        # Setup mocks
        mock_project_manager_instance = MagicMock()
        mock_project_manager.return_value = mock_project_manager_instance
        ...
        mock_console.print.assert_any_call("[red]Network error while generating concept: Network error[/red]")
        
    @patch('create_book_command.ProjectManagerAgent')
    @patch('create_book_command.Settings')
    @patch('create_book_command.console')
    def test_create_book_with_all_validation_parameters(self, mock_console, mock_settings, mock_project_manager):
        # Setup mocks
        mock_project_manager_instance = MagicMock()
        mock_project_manager.return_value = mock_project_manager_instance
        mock_project_manager_instance.project_dir = Path("/mock/project/dir")
