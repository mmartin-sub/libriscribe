...
from libriscribe2.config import EnvironmentConfig, load_model_config
...
def test_env_file_loading():
    ...
    try:
        ...
        EnvironmentConfig(config_file=temp_env_file)
...
def test_settings_integration():
    ...
    try:
        ...
        # Create a config file with the test values instead of setting environment variables
        config_data = {
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as config_f:
            json.dump(config_data, config_f)
            temp_integration_config = config_f.name

        settings = Settings(config_file=temp_integration_config)
        ...
        # Check model configuration with the config file
        model_config = settings.get_model_config(temp_config_file)
        ...
        os.unlink(temp_env_file)
        os.unlink(temp_config_file)
        if "temp_integration_config" in locals():
            os.unlink(temp_integration_config)
