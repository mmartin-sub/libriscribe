...
def normalize_language(lang_value: str | None) -> str | None:
    """Normalize language names to BCP 47 codes using langcodes."""
    if not lang_value:
        return lang_value
    try:
        # langcodes standardizes names and codes
        tag_result = langcodes.find(lang_value).to_tag()
        return str(tag_result) if tag_result is not None else lang_value
    except Exception:
        return lang_value  # fallback to original if not recognized


def generate_yaml_metadata(project_knowledge_base: Any, write_to_file: bool = True) -> str:
    """
    ...
    for src_key, template_key in override_map.items():
        ...
        if value is not None and value not in ["", []]:
            if template_key == "keywords" and isinstance(value, str):
                ...
                lang_value = get_field("lang")
                context[template_key] = str(lang_value) if lang_value is not None else "en"  # long format
                ...
                context[template_key] = [value]
            elif template_key == "author" and isinstance(value, list):
                # It's already a list, so just use it.
                context[template_key] = value
            elif template_key == "author":
                # Handle None or other types by converting to list
                context[template_key] = [str(value)] if value is not None else ["Unknown Author"]
            else:
                context[template_key] = value
    ...
    try:
        ...
        print("[yellow]   Context provided to template:[/yellow]")
    ...
    print("[LOG] YAML metadata updated (Jinja2)")
