...
class ValidationTestFramework:
    ...
    def __init__(self, 
        ...
        self.test_data_dir = Path(test_data_dir) if test_data_dir else Path(".libriscribe/test_data")
    ...
    async def create_comprehensive_test_suite(self, 
                                            validators: List[str],
                                            content_types: List[str]) -> List[TestCase]:
        ...
        for validator_id in validators:
            for content_type in content_types:
                ...
                for scenario in MockScenario:
                    test_case = await self._create_test_case_for_scenario(
                        validator_id, content_type, scenario
                    )
                    test_cases.append(test_case)
                    
                # Create accuracy test cases with known-good/bad content
                accuracy_cases = await self._create_accuracy_test_cases(
                    validator_id, content_type
                )
    ...
    async def _create_test_case_for_scenario(self,
                                           validator_id: str,
                                           content_type: str,
                                           scenario: MockScenario) -> TestCase:
        """Create test case for specific scenario"""
        
        # Generate appropriate test data
        test_data = await self.test_data_generator.generate_content(
            content_type, scenario
        )
        
        # Set expected results based on scenario
        expected_status, expected_findings, quality_range = \
            self._get_expected_results_for_scenario(scenario)
    ...
    async def _create_accuracy_test_cases(self,
                                        validator_id: str,
                                        content_type: str) -> List[TestCase]:
        """Create test cases for accuracy measurement"""
        
        accuracy_cases = []
        
        # Known-good content (should pass validation)
        good_content = await self.test_data_generator.generate_known_good_content(
            content_type, validator_id
        )
        ...
        bad_content = await self.test_data_generator.generate_known_bad_content(
            content_type, validator_id
        )
    ...
    def _get_expected_results_for_scenario(self, scenario: MockScenario) -> tuple:
    ...
    async def _create_mock_validator(self, test_case: TestCase) -> ValidatorBase:
        ...
        class MockValidator(ValidatorBase):
            def __init__(self, test_case: TestCase, mock_manager: AIMockManager):
                super().__init__(
                    test_case.validator_id,
                ...
                try:
                    ...
                    if "findings" in response_data:
                        for finding_data in response_data["findings"]:
                            finding = Finding(
                                validator_id=self.validator_id,
                                type=finding_data.get("type", "content_quality"),
                    ...
                    from ..interfaces import ValidatorResult
